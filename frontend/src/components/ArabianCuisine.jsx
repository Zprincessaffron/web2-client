...
const ArabianCuisine = () => {
  const [recipes, setRecipes] = useState([]);
  const [error, setError] = useState(null);
  const [selectedRecipe, setSelectedRecipe] = useState(null);
  const [expandedRecipeId, setExpandedRecipeId] = useState(null); // New state for expanded recipe ID
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/api/arabian-cuisine');
  ...
  const toggleRecipeDetails = async (recipe) => {
    ...
    try {
      await axios.post('/api/store-interaction', {
        ...
        useCase: 'culinary',
        cuisine: 'Arabian Cuisine' // Update the cuisine type if needed
      });
  ...
  const Modal = ({ isOpen, onClose, message }) => {
    ...
    return (
      ...
      <div className="bg-custom-gradient-8 p-8 rounded-lg shadow-lg md:w-full w-[90%] max-w-lg relative">
      <button
          onClick={onClose}
        ...
        >
          ...
          >
            <button
              ...
              onClick={() => setSelectedRecipe(null)} // Update this to call `closeRecipeDetails` or `toggleRecipeDetails`
            >
